...
public class MaxElementInArrayBinarySearch {
    ...
    static int findMaximum(int arr[], int low, int high) {
        ...
        if ( arr[mid] > arr[mid + 1] && arr[mid] > arr[mid - 1])
            return arr[mid];
        /* If arr[mid] is greater than the next element and smaller than the previous element then maximum lies on left side
          of mid */
        if (arr[mid] > arr[mid + 1] && arr[mid] < arr[mid - 1])
